...
structure definition $D$ in~the~input graph $G_{input}$. The~equivalent for~$C_1$ is~in~Figure~\ref{fig:output-graph} denoted as~$R_{i,1}$. But there are many other 
...
\begin{figure}
	...
	That means one concrete instance $R_{i,1}$ for component $C_1$. Such a match is~generally denoted
	as $M_1$.}
...
kinds of~dimensions. The~first one will express the~time of~the~measurement, the~second one will cover its location. It~will support one measure that has to~be~a number.
