...
~\cite{ontowiki}. The OLAP2DataCube tool is in fact a plugin for a well--known 
...
large--scale application, developed by this group, called the Open Data Portal of 
...
As well as the OLAP2DataCube tool, the CubeViz is also based on the 
OntoWiki~\cite{ontowiki} application. In fact, it adds another layer to the 
...
order to provide data visualization (an example can be seen in Figure~\{fig:cubeviz}).
...
When speaking about the Details-on-demand feature, we would like to distinguish
